#!/bin/bash
# gsutil wrapper (macOS version) — thin client that sends commands to the host
# proxy via Unix socket.
# This script lives INSIDE the container. The real gsutil runs OUTSIDE.
#
# macOS difference: socket at /tmp/gsutil-proxy.sock instead of /var/run/
set -euo pipefail
...
if [ ! -S "${SOCKET_PATH}" ]; then
    ...
    echo "" >&2
    echo "On macOS, start it with:" >&2
    echo "  python3 gsutil-proxy.py --socket /tmp/gsutil-proxy.sock" >&2
    exit 1
fi
